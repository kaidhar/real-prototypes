...
function showHelp() {
  ...
  console.log(`
\x1b[1mUSAGE\x1b[0m
  platform-prototype <command> [options]
...
\x1b[1mEXAMPLES\x1b[0m
  ...
  platform-prototype new --project my-app

  # Capture a platform
  platform-prototype capture --project my-app --url https://app.example.com --email user@test.com --password secret

  # Validate capture
  platform-prototype validate --project my-app --phase post-capture

  # Generate prototype
  platform-prototype generate --project my-app

  # Run full pipeline
  platform-prototype pipeline --project my-app --url https://app.example.com --email user@test.com --password secret

  # List all projects
  platform-prototype list
...
function requireProject(options, command) {
  if (!options.project) {
    ...
    log('Example: platform-prototype ' + command + ' --project my-app', 'info');
    process.exit(1);
  }
}
...
function runNew(options) {
  ...
  if (!options.project) {
    ...
    log('Example: platform-prototype new --project my-app', 'info');
    process.exit(1);
  }
...
\x1b[1mNext Steps:\x1b[0m
  1. Capture a platform:
     platform-prototype capture --project ${options.project} --url https://your-platform.com

  2. Or run the full pipeline:
     platform-prototype pipeline --project ${options.project} --url https://your-platform.com
  `);
}
...
function runList() {
  ...
  if (!fs.existsSync(PROJECTS_DIR)) {
    log('No projects found. Create one with: platform-prototype new --project <name>', 'info');
    return;
  }
...
\x1b[1mGeneration Instructions:\x1b[0m
  ...
  4. Validate with: platform-prototype validate --phase post-gen
...
\x1b[1mNext Steps:\x1b[0m
  ...
  4. Run: platform-prototype validate --phase post-gen
  `);
}
...
\x1b[1mNext Steps:\x1b[0m
  ...
  3. Run: platform-prototype capture --config ${outputPath}
  `);
}
...
switch (options.command) {
  ...
  default:
    ...
    log('Run "platform-prototype --help" for usage', 'info');
    process.exit(1);
}
